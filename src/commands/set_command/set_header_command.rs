...
impl SetHeaderCommand {
    pub fn get_command() -> Command {
        Command::new("header")
            .about("Set a header for a service or environment")
            .arg(
                Arg::new("name")
                    .help("The header name to set")
                    .required(true)
            )
            .arg(
                Arg::new("value")
                    .help("The header value to set")
                    .required(true)
            )
            .arg(
                Arg::new("service")
    ...
    pub fn bind_from_matches(args: &ArgMatches) -> SetHeaderCommand {
        SetHeaderCommand {
            header_name: args.bind_field("name"),
            header_value: args.bind_field("value"),
    ...
    pub fn execute(&self, config: &mut HtrsConfig) -> Result<HtrsAction, HtrsError> {
        ...
        header_item.set_header(self.header_name.clone(), self.header_value.clone());
        Ok(UpdateConfig)
    }
}
